...
#include "storage/latch.h"                                              // for correct sleep
#include "storage/lwlock.h"
#include "storage/proc.h"                                               // myProc should be defined here
...
/*
* To ensure that a dynamically loaded object file is not loaded into an incompatible server, 
* PostgreSQL checks that the file contains a "magic block" with the appropriate contents. 
* This allows the server to detect obvious incompatibilities, such as code compiled for 
* a different major version of PostgreSQL.
*/
PG_MODULE_MAGIC;

static char *  stats_recorder_schema = "stats_recorder";
static int     stats_recorder_naptime = 1;                              // Duration between each statistics gathering (in seconds)
static bool    debug = true;

void	_PG_init(void);													// Defenition at the end of the file

/* 
 * Signal handling. Which function to call(connect) for current signal
 *  (SIGTERM or SIGHUP)  
 */
...
/*
  ...
  * SetLatch in the signal handler.
  * SetLatch     - Sets the latch
  * ResetLatch   - Clears the latch, allowing it to be set again
  * WaitLatch    - Waits for the latch to become set
  * For more details http://www.postgresql.org/message-id/attachment/10723/unix_latch.c
 */
 
/*
 * If process (worker) is  sleeping, command him to wake up  (for correct SIGHTERM signal usage)
 * The sleep time previously invoked with WaitLatch is stopped with SetLatch immediately
*/
	if (MyProc)
...
initialize_stats_recorder_spi()
{
	...
	if (debug)
		elog(LOG, "%s, initialize_stats_recorder_spi", MyBgworkerEntry->bgw_name); //elog wrii
...
_PG_init(void)
{
    ...
    * About GUC you can read here http://www.enterprisedb.com/postgres-plus-edb-blog/bruce-momjian/what-guc-variable
    * and here http://www.postgresql.org/docs/current/static/config-setting.html
	*/
	...
	worker.bgw_flags = BGWORKER_SHMEM_ACCESS |                                     // The process is  allowed access to shared memory with the flag BGWORKER_SHMEM_ACCESS
		BGWORKER_BACKEND_DATABASE_CONNECTION;
	worker.bgw_start_time = BgWorkerStart_RecoveryFinished;                        // bgw_start_time is set to start only once the system has reached a stable read-write state
